...
def create_github_pr(repo_name: str, file_path: str, fixed_code: strimport os
from github import Github, GithubException
...
def create_github_pr(repo_name: str, file_path: str, fixed_code: str, commit_message: str):
    ...
    try:
        ...
        return None, commit_message: str):
    token = os.getenv("GITHUB_TOKEN")
    if not token:
        print("[Git Error] GITHUB_TOKEN environment variable is missing.")
        return None
    ...
    try:
        ...
        # 1. Get reference to default branch (usually 'main')
        default_branch = repo.default_branch
        sb = repo.get_branch(default_branch)
        ...
        try:
            ...
            # File doesn't exist yet in GitHub repo, create it anew
            repo.create_file(
                path=file_path,
                message=commit_message,
                content=fixed_code,
        ...
        # 4. Open Pull Request from new_branch into main
        pr = repo.create_pull(
            title=commit_message,
            body="🤖 **Automated CI/CD Fix**: Generated and verified by Gemini AI Doctor inside Docker Sandbox.",
            head=new_branch_name,
